...
const answerSchema = {
  ...
  properties: {
    ...
    cauTraLoi: { type: 'string', description: 'Cau tra loi cuoi cung danh cho hoc vien.' },
...
function validPages(value) {
  ...
  const hopLe = pages.every(
    (page) => Number.isInteger(page.so) && page.so > 0 && page.tieuDe && page.noiDung
  );
...
export function chuanHoaKetQua(raw, trangTuTool) {
  ...
  let nuocDi = NUOC_DI_HOP_LE.includes(raw?.nuocDi) ? raw.nuocDi : 'chua_co_can_cu';
  ...
  if (!cauTraLoi) {
    ...
    cauTraLoi = 'Minh chua tra loi duoc cau nay. Ban thu hoi lai hoac hoi giang vien va TA.';
    canhBao.push('cauTraLoi rong');
  }

  if (nuocDi === 'chua_co_can_cu') {
    if (soTrang !== null) canhBao.push('chua_co_can_cu nhung co soTrang, da bo');
    soTrang = null;
...
async function createResponse(input) {
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      tools: tutorTools,
      text: {
        format: { type: 'json_schema', name: 'tutor_answer', strict: true, schema: answerSchema }
      }
    })
  });
  if (!response.ok) {
    throw new Error(`OpenAI API ${response.status}: ${(await response.text()).slice(0, 300)}`);
  }
...
async function answerQuestion(body) {
  ...
  for (let turn = 0; turn < 3; turn += 1) {
    ...
    if (!calls.length) {
      let raw;
      try {
        raw = JSON.parse(getOutputText(response));
      } catch {
    ...
    for (const call of calls) {
      ...
      try {
        ...
        result =
          call.name === 'search_course_material'
            ? searchCourseMaterial(args, pages)
            : { error: 'Tool khong duoc phep.' };
        for (const item of result.results || []) trangTuTool.push(item.page);
        toolTrace.push({
          turn: turn + 1,
          tool: call.name,
          arguments: args,
...
async function serveStatic(res, ten) {
  try {
    ...
    if (!duongDan.startsWith(path.join(root, 'codebase'))) return json(res, 404, { error: 'Not found' });
    const noiDung = await readFile(duongDan);
    res.writeHead(200, { 'Content-Type': KIEU_FILE[path.extname(ten)] || 'text/plain; charset=utf-8' });
    res.end(noiDung);
  } catch {
...
const server = http.createServer(async (req, res) => {
  try {
    if (req.method === 'GET') {
      if (req.url === '/' || req.url === '/index.html') return serveStatic(res, 'index.html');
      if (/^\/[\w.-]+\.(js|css)$/.test(req.url)) return serveStatic(res, req.url.slice(1));
    }
    if (req.method !== 'POST' || req.url !== '/api/tra-loi') {
      return json(res, 404, { error: 'Not found' });
    }
    if (!process.env.OPENAI_API_KEY) {
      return json(res, 503, { error: 'Thieu OPENAI_API_KEY. Hay tao file .env tu .env.example.' });
    }

    let raw = '';
    for await (const chunk of req) {
      raw += chunk;
      if (raw.length > 100000) return json(res, 413, { error: 'Request qua lon.' });
    }
    return json(res, 200, await answerQuestion(JSON.parse(raw)));
  } catch (error) {
    console.error(error.message);
    return json(res, 400, { error: error.message || 'Khong the xu ly cau hoi.' });
  }
});
