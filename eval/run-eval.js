...
const codeTaiLieu = await readFile(path.join(root, 'codebase', 'course-data.js'), 'utf8');
...
async function chayMotCase(c) {
  ...
  try {
    ...
    if (!res.ok) {
      return { loi: data.error || `HTTP ${res.status}`, ms: Date.now() - batDau };
    }
    return {
...
function chamDiem(c, kq) {
  ...
  const trangDung =
    c.trangMongDoi === null
      ? kq.trangThucTe === null
      : Array.isArray(c.trangMongDoi) && c.trangMongDoi.includes(kq.trangThucTe);
...
function bangMarkdown(rows, tomTat) {
  const dong = (r) =>
    `| ${r.id} | ${r.loai} | ${r.cauHoi.replace(/\|/g, '/')} | ${r.nuocDiMongDoi} | ${
      r.nuocDiThucTe ?? 'LOI'
...
| ID | Loai | Cau hoi | Nuoc di mong doi | Nuoc di thuc te | Trang mong doi | Trang thuc te | Ket qua | Ghi chu |
|---|---|---|---|---|---|---|---|---|
${rows.map(dong).join('\n')}
`;
}
...
/* ---- Main ---- */
const bo = JSON.parse(await readFile(path.join(here, 'golden-set.json'), 'utf8'));
const rows = [];
...
for (const c of bo.cases) {
  ...
  rows.push({
    ...
    trangMongDoi: Array.isArray(c.trangMongDoi) ? c.trangMongDoi.join(' hoac ') : c.trangMongDoi,
...
await writeFile(path.join(here, 'ket-qua.md'), bangMarkdown(rows, tomTat), 'utf8');
await writeFile(path.join(here, 'ket-qua.json'), JSON.stringify({ tomTat, rows }, null, 2), 'utf8');
