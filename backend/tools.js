...
const TU_DUNG = new Set([
  'la', 'gi', 'the', 'nao', 'cho', 'toi', 'minh', 'cua', 'va', 'co', 'khong',
  'mot', 'trong', 've', 'nay', 'do', 'ban', 'hay', 'duoc', 'thi', 'ma', 'nhu',
  'voi', 'de', 'giai', 'thich', 'hoi', 'noi', 'cai', 'cac', 'nhung', 'ra',
  'len', 'tren', 'khi', 'neu', 'sao', 'vay', 'a', 'em', 'anh', 'chi', 'oi',
  'nhe', 'day', 'kia', 'lam', 'dung', 'viec', 'cach', 'nghia', 'tuc', 'boi'
]);
...
export const tutorTools = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        selected_text: {
          type: 'string',
          description: 'Doan nguoi hoc dang boi den. Neu khong co thi truyen chuoi rong.'
        },
        current_page: {
          type: 'integer',
...
export function searchCourseMaterial(args, pages) {
  const { query = '', selected_text: selectedText = '', current_page: currentPage = -1 } = args || {};

  // Hoc vien noi ro so trang thi uu tien dung trang do.
  const noiSoTrang = normalize(query).match(/trang\s*(\d+)/);
  if (noiSoTrang) {
    const trang = pages.find((page) => Number(page.so) === Number(noiSoTrang[1]));
    if (trang) {
      return {
        found: true,
        results: [{ page: trang.so, title: trang.tieuDe, excerpt: trang.noiDung, score: 999 }]
      };
    }
  }
  ...
  const results = pages
    .map((page) => {
      ...
      if (selectedText && normContent.includes(normalize(selectedText))) score += 40;
      if (Number(currentPage) > 0 && Number(page.so) === Number(currentPage)) score += 10;
